...
class IceCatUpdateProduct
{
    ...
    {
                $writer = new \Zend_Log_Writer_Stream(BP . '/var/log/IceCatUpdateProduct.log');
                $logger = new \Zend_Log();
        ...
        if ($this->data->isReasonToBuyEnabled()) {
            $logger->info("Inicio ReasonToBuy");
...
$logger->info("fin con Images");
                }
            }
           $logger->info("fin sin Images");

        }

        if ($this->data->isImportMultimediaEnabled()) {
$logger->info("Inicio Multimedia");
                        ...
                        if (strpos($multiMediaData['URL'], 'youtube') !== false) {
                            ...
                            if(strpos($headers[0], '200')) {
                                ...
                                if ($product->hasGalleryAttribute()) {
                                    ...
                                    $product->save();
$logger->info("fin con Multimedia");
...
$logger->info("fin sin Multimedia");

        }

        if ($this->data->isImportPdfEnabled()) {
$logger->info("inicio Pdf");
...
$logger->info("fin con Pdf");
                    }
                }
            }
$logger->info("fin sin Pdf");
        }
    
        if ($this->data->isImportSpecificationEnabled()) {
$logger->info("inicio Specification");
...
$logger->info("inicio RelatedProduct");
		...
		$logger->info("fin con RelatedProduct");
                if (count($linkData) > 0) {
                    $product->setProductLinks($linkData);
                }
            }
		$logger->info("fin sin RelatedProduct");
        }
	$logger->info("setData");
        $product->setData('icecat_updated_time', date('Y-m-d H:i:s'));
	$logger->info("fin setData");

        $product->setIcecatRun(1);
        $this->productResource->save($product);
	$logger->info("fin del icecatRun");
